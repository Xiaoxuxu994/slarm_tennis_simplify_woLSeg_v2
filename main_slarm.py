...
from src.dataset.datasets import (
    CustomConcatDataset,
    PerceptualModelDataset,
    PerceptualModelDatasetEval,
    Stream25Dataset,
    Stream25EvalDataset,
)
...
def _resolve_annotation_path(data_root, annotation_path):
    if annotation_path is None or os.path.isabs(annotation_path):
        return annotation_path
    return os.path.join(data_root, annotation_path)


def _select_dataset_classes(stream25_reconstruction_loss=False):
    if stream25_reconstruction_loss:
        return Stream25Dataset, Stream25EvalDataset
    return PerceptualModelDataset, PerceptualModelDatasetEval


def _select_annotations(args, dataset_meta, dataset_name):
    if args.stream25_reconstruction_loss:
        return args.train_annotation, args.eval_annotation
    return dataset_meta["annotation_txt_file_train"], dataset_meta["annotation_txt_file_val"]


def _build_train_sampler(dataset, args, seed, world_size=None, rank=None):
    """Build the production sampler for the selected training contract."""
    if args.stream25_reconstruction_loss:
        return Stream25StepSampler(
            num_scenes=len(dataset),
            seed=seed,
            start_step=args.start_iteration,
            num_replicas=world_size,
            rank=rank,
            shuffle=True,
        )
    return InfiniteSampler(
        sample_count=len(dataset) * (world_size or torch.distributed.get_world_size()),
        shuffle=True,
        seed=seed,
    )


def _build_optimizer_param_groups(model, args):
    """Select the production optimizer partition for the active experiment."""
    if args.stream25_reconstruction_loss:
        return make_stream25_param_groups(
            model,
            head_lr=args.lr,
            trunk_lr=args.stream25_trunk_lr,
            weight_decay=args.weight_decay,
        )
    return optim_factory.param_groups_weight_decay(model, args.weight_decay)
...
def get_args_parser():
    ...
    # =============== Stream25 streaming-reconstruction base =============== #
    ...
    parser.add_argument("--enable_task_semantic_head", action="store_true",
                        help="add a separate four-class Gaussian task-semantic head (kept apart from LSeg)")
    parser.add_argument("--terminal_context_extrapolation", action="store_true",
                        help="terminal context owns extrapolation through frame 24")
    parser.add_argument(
        "--stream25_render_target_chunk_size",
        type=int,
        default=20,
        help="maximum target timestamps rendered together (window-6 uses 1)",
    )
    # Differential LR: aggregator / patch-embed / shared trunk step slower than the heads (spec 6.3).
    parser.add_argument("--stream25_trunk_lr", type=float, default=2.0e-6,
                        help="LR for aggregator / patch embedding / shared latent norm (heads use --lr)")
    # The ten weighted optimization terms summed directly into the total (spec 6.2).
    parser.add_argument("--stream25_rgb_weight", type=float, default=1.00)
    parser.add_argument("--stream25_lpips_weight", type=float, default=0.05)
    parser.add_argument("--stream25_ball_rgb_weight", type=float, default=0.50)
    parser.add_argument("--stream25_depth_relative_weight", type=float, default=1.00)
    parser.add_argument("--stream25_ball_depth_metric_weight", type=float, default=0.50)
    parser.add_argument("--stream25_semantic_weight", type=float, default=1.00)
    parser.add_argument("--stream25_lseg_feature_weight", type=float, default=1.00)
    ...
    parser.add_argument("--stream25_ms3_jerk_scale", type=float, default=1.0)
    parser.add_argument("--stream25_ms3_tail_fraction", type=float, default=0.10,
                        help="largest MS3 error fraction receiving explicit tail pressure")
    parser.add_argument("--stream25_ms3_tail_weight", type=float, default=1.0,
                        help="weight of the MS3 top-fraction mean added to the mean loss")
    parser.add_argument("--stream25_ball_depth_tail_fraction", type=float, default=0.10,
                        help="largest ball-depth error fraction receiving tail pressure")
    parser.add_argument("--stream25_ball_depth_tail_weight", type=float, default=0.50,
                        help="weight of the ball-depth top-fraction mean")
    parser.add_argument("--terminal_dynamic_velocity_threshold", type=float, default=1.0,
                        help="||v6|| >= this (m/s) marks a terminal Gaussian dynamic for extrapolation")
    parser.add_argument("--stream25_semantic_class_weight_cap", type=float, default=10.0,
                        help="cap on the inverse-sqrt train-frequency semantic class weights")
    ...
    parser.add_argument(
        ...
        type=float,
        default=None,
        help="physical reconstruction interval in seconds; must be explicit",
    )
    ...
    parser.add_argument("--strict_data_loading", action="store_true")
    parser.add_argument(
        "--train_annotation",
        default="scene_list/ball_catch_24cm_stereo40_stream25_nopitch_train.txt",
        type=str,
    )
    parser.add_argument(
        "--eval_annotation",
        default="scene_list/ball_catch_24cm_stereo40_stream25_nopitch_validation.txt",
        type=str,
    )
    ...
    # ============= Config file ============= #
    # Optional YAML config applied as argparse defaults via parse_args_with_yaml_config.
    # Explicit CLI flags always override the YAML values; absent --config == legacy behavior.
    parser.add_argument("--config", default=None, type=str,
                        help="path to a YAML config whose keys set argparse defaults")

    return parser


def _move_feature_extractor(extractor, device):
    """Move LSeg modules and clear retained hook activations."""
    if extractor is None:
        return None
    activations = getattr(extractor.model_pretrained, "activations", None)
    if isinstance(activations, dict):
        activations.clear()
    extractor.model_pretrained = extractor.model_pretrained.to(device)
    extractor.model_scratch = extractor.model_scratch.to(device)
    return extractor


def _checkpoint_due(args, data_iter_step: int) -> bool:
    return (data_iter_step + 1) % args.ckpt_every_n_iters == 0


def _visualization_samples_for_step(args, data_iter_step: int) -> int:
    """Guarantee one train/val video pair for every Stream25 checkpoint."""
    periodic_due = (data_iter_step + 1) % args.vis_every_n_iters == 0
    checkpoint_video_due = (
        args.stream25_reconstruction_loss and _checkpoint_due(args, data_iter_step)
    )
    if not periodic_due and not checkpoint_video_due:
        return 0
    if checkpoint_video_due:
        return max(1, int(args.num_vis_samples))
    return max(0, int(args.num_vis_samples))


def main(args):
    # Stage-A single-GPU runs bypass torchrun/NCCL entirely. Multi-process
    # entry points retain the existing distributed initialization path.
    if os.environ.get("SLARM_SINGLE_PROCESS") == "1":
    ...
    dataset_class, eval_dataset_class = _select_dataset_classes(
        args.stream25_reconstruction_loss
    )

    for dataset_name in args.dataset:
        dataset_meta = DATASET_DICT[dataset_name]
        train_annotation, val_annotation = _select_annotations(args, dataset_meta, dataset_name)

        if train_annotation is not None:
            if dataset_name == "nuscenes":
                train_annotation = f"data/dataset_scene_list/nuscenes_train.txt"
            else:
                train_annotation = _resolve_annotation_path(
                    args.data_root, train_annotation
                )
        ...
        if val_annotation is not None:
            if dataset_name == "nuscenes":
                ...
                val_annotation = _resolve_annotation_path(args.data_root, val_annotation)
        ...
        if val_annotation is not None:
            ...
            val_datasets.append(dataset_class(
                data_root=args.data_root,
            ...
            eval_datasets.append(eval_dataset_class(
                data_root=args.data_root,
                annotation_txt_file_list=val_annotation,
                target_size=args.input_size,
                num_context_timesteps=args.num_context_timesteps,
            ...
            eval_flow_datasets.append(eval_dataset_class(
                data_root=args.data_root,
                annotation_txt_file_list=val_annotation,
                target_size=args.input_size,
                num_context_timesteps=args.num_context_timesteps,
    ...
    sampler_train = _build_train_sampler(
        dataset_train,
        args,
        seed,
    ...
    param_groups = _build_optimizer_param_groups(model_without_ddp, args)
    ...
    if isinstance(sampler_train, Stream25StepSampler):
        sampler_train.start_step = args.start_iteration
    ...
    _move_feature_extractor(feat_extractor, "cpu")
    torch.cuda.empty_cache()
    rgb_and_lpips_loss = RGBLpipsLoss(
        perceptual_weight=(1.0 if args.stream25_reconstruction_loss else args.perceptual_weight),
    ...
    for data_dict in metric_logger.log_every(
        ...
        rgb_and_lpips_loss = rgb_and_lpips_loss.to("cpu")
        _move_feature_extractor(feat_extractor, device)
        torch.cuda.empty_cache()
        with torch.autocast("cuda", dtype=dtype):
            ...
            input_dict, target_dict = prepare_inputs_and_targets(data_dict, device, v=num_max_cams,
                                                                 ...
                                                                 feat_extractor=feat_extractor)
            _move_feature_extractor(feat_extractor, "cpu")
            rgb_and_lpips_loss = rgb_and_lpips_loss.to(device)
            torch.cuda.empty_cache()
            if data_iter_step in time_count_step:  # time debug
                torch.cuda.synchronize()
                print('##################')
        ...
        checkpoint_due = _checkpoint_due(args, data_iter_step)
        visualization_samples = _visualization_samples_for_step(args, data_iter_step)

        # Release the completed training graph before checkpoint-associated
        # visualization temporarily brings the LSeg teacher back to CUDA.
        del pred_dict, input_dict, target_dict, loss_dict, loss_value
        ...
        if visualization_samples > 0:
            if distributed.is_main_process():
                ...
                torch.cuda.empty_cache()
                try:
                    for _ in range(visualization_samples):
                        vis_slice_id, valid_slice_id = visualize(
                            args=args,
                            model=model_without_ddp,
                            dset_train=dataset_train,
        ...
        # For Stream25, save only after its required train/val video pair has
        # completed successfully, so every checkpoint is accompanied by videos.
        if checkpoint_due:
            if distributed.is_main_process():
                ...
                if args.stream25_reconstruction_loss:
                    checkpoint["stream25_contract"] = (
                        misc.stream25_checkpoint_contract(args)
                    )
...
if __name__ == "__main__":
    ...
    args = parse_args_with_yaml_config(get_args_parser())
    main(args)
