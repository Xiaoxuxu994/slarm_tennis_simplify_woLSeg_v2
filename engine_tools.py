...
def evaluation_target_positions(context_frame_ids, target_frame_ids):
    """Return target tensor slots whose absolute frame IDs are not context frames."""
    context_frame_ids = set(context_frame_ids)
    return [
        position
        for position, frame_id in enumerate(target_frame_ids)
        if frame_id not in context_frame_ids
    ]
...
def evaluate(dataloader, model, args, name_str=None):
    ...
    for data_dict in pbar:
        ...
        context_frame_ids = input_dict["context_frame_idx"][0].cpu().numpy().tolist()
        target_frame_ids = target_dict["target_frame_idx"][0].cpu().numpy().tolist()
        test_positions = evaluation_target_positions(
            context_frame_ids,
            target_frame_ids,
        )
        test_frame_ids = [target_frame_ids[position] for position in test_positions]
        if not printed:
            logger.info(f"Input frame IDs: {context_frame_ids}")
            logger.info(f"Test frame IDs: {test_frame_ids}")
            printed = True
        pred_dict = model(input_dict)
        # evaluate on real target images:
        # b, t, v, c, h, w
        gt_rgb = target_dict["target_image"][:, test_positions]
        # b, t, v, h, w, c
        ...
        gt_depth = target_dict["target_depth"][:, test_positions].view(-1, height, width)
        gt_sky_mask = target_dict["target_sky_masks"][:, test_positions].view(-1, height, width)
        occupied_mask = (gt_sky_mask == 0).bool()
        if "target_dynamic_masks" in target_dict:
            gt_dynamic_mask = target_dict["target_dynamic_masks"][:, test_positions]
        ...
        # pred_rgb = rendered_results[rendered_results["rgb_key"]][:, test_positions] * std + mean
        pred_rgb = rendered_results[rendered_results["rgb_key"]][:, test_positions]
        ...
        if rendered_results["decoder_depth_key"] is None:
            pred_depth = rendered_results[rendered_results["depth_key"]][:, test_positions].view(
                -1, height, width
            )
        else:
