...
class Aggregator(nn.Module):
    ...
    def forward(
        ...
        if kv_cache_list is not None and t != 1:
            pass

        # A masked monolithic SDPA call and incremental cached SDPA use
        # different kernels/reduction shapes; in bf16 their error compounds
        # through 12 layers enough to change geometry materially.  Train and
        # batch-eval window inputs through the exact incremental computation
        # used by StreamSession so the deployed causal path is what is learned.
        if kv_cache_list is None and t > 1 and mode.startswith("window_"):
            window_size = int(mode.split("_")[-1])
            if t <= window_size:
                cache = [[None, None] for _ in range(self.depth)]
                ...
                for timestep in range(t):
                    step_data = dict(data_dict)
                    for key in temporal_keys:
                        value = data_dict.get(key)
                        if isinstance(value, torch.Tensor) and value.shape[1] == t:
                            step_data[key] = value[:, timestep:timestep + 1]
                    step_outputs, patch_start_idx, cache = self.forward(
                        step_data, mode=mode, kv_cache_list=cache
                    )
                    if per_layer_outputs is None:
                        per_layer_outputs = [[] for _ in step_outputs]
                    for layer, output in enumerate(step_outputs):
                        if output is not None:
                            per_layer_outputs[layer].append(output)
                combined = [
                    None if not pieces else torch.cat(pieces, dim=1)
                    for pieces in per_layer_outputs
                ]
                return combined, patch_start_idx
