...
from .temporal_ownership import (
    classify_terminal_dynamic,
    ms3_acceleration,
    ms3_displacement,
    ms3_velocity,
)
...
def _select_latest_perception_tokens(
    aggregated_tokens: torch.Tensor,
    *,
    num_observations: int,
    num_views: int,
) -> torch.Tensor:
    """Return the normalized patch tokens for the latest observation."""
    if aggregated_tokens.ndim != 4:
        pass
    batch, temporal_views, patches, channels = aggregated_tokens.shape
    if temporal_views != num_observations * num_views:
        pass
    return aggregated_tokens.reshape(
        batch,
        num_observations,
        num_views,
        patches,
...
class SLARM(nn.Module, PyTorchModelHubMixin):
    ...
    def __init__(
        ...
        emit_terminal_perception_tokens=False,
        enable_task_semantic_head=False,
        terminal_context_extrapolation=False,
        terminal_dynamic_velocity_threshold=1.0,
        render_target_chunk_size=20,
        ...
        **kwargs,
        ...
        self.emit_terminal_perception_tokens = emit_terminal_perception_tokens
        self.enable_task_semantic_head = enable_task_semantic_head
        self.terminal_context_extrapolation = terminal_context_extrapolation
        self.terminal_dynamic_velocity_threshold = terminal_dynamic_velocity_threshold
        if not isinstance(render_target_chunk_size, int) or render_target_chunk_size <= 0:
            pass
        self.render_target_chunk_size = render_target_chunk_size
        if self.enable_task_semantic_head and not self.use_last_token:
            pass
        ...
        if self.emit_terminal_perception_tokens:
            if not self.use_last_token:
                pass
            if self.num_cams != 3:
                pass
            if self.patch_size != 8:
                pass
            if tuple(self.img_size) != (320, 240):
                pass
            if 2 * self.embed_dim != 1536:
                pass
            if not self.terminal_context_extrapolation:
                pass
            if self.mode != "window_6":
                pass
        ...
        if self.use_last_token:
            ...
            if self.enable_task_semantic_head:
                self.task_semantic_pred = nn.Linear(
    ...
    def forward_task_semantic_predictor(self, x, shape):
        h, w, t, v = shape
        x = rearrange(x, "b tv hw c -> (b tv) hw c")
        logits = self.task_semantic_pred(x)
        logits = self.unpatchify(logits, hw=(h, w), patch_size=self.unpatch_size)
        logits = rearrange(logits, "(b t v) c h w -> b t v h w c", t=t, v=v)
        return logits
    ...
    def forward_renderer(self, gs_params, data_dict, feats=None, task_semantic=None, render_motion_seg=not is_ascend_npu(),
        ...
        task_semantic_flat = rearrange(task_semantic, "b t v h w c -> b (t v h w) c") if task_semantic is not None else None
        ...
        task_semantic_batched = repeat(task_semantic_flat, "b ... -> (b t) ...", t=tgt_t) if task_semantic_flat is not None else None
        ...
        if not self.use_ms3_motion:
            ...
            ms3_v0 = forward_ms3_batched[..., 0:3]
            ms3_a0 = (
                forward_ms3_batched[..., 3:6]
                if self.ms3_deg >= 2
                else torch.zeros_like(ms3_v0)
            )
            ms3_j0 = (
                forward_ms3_batched[..., 6:9]
                if self.ms3_deg >= 3
                else torch.zeros_like(ms3_v0)
            )
            target_ms3_batched = torch.cat(
                [
                    ms3_velocity(ms3_v0, ms3_a0, ms3_j0, tdiff_forward_batched),
                    ms3_acceleration(ms3_a0, ms3_j0, tdiff_forward_batched),
                    ms3_j0,
                ],
                dim=-1,
            )
            ...
            terminal_displacement = None
            if self.terminal_context_extrapolation and ("window" in self.mode or "causal" in self.mode):
                terminal_start = (t - 1) * v * h * w
                terminal_end = t * v * h * w
                terminal_tdiff = tdiff_forward_batched[:, terminal_start:terminal_end, :]
                terminal_positive_bool = terminal_tdiff > -0.5 * delta_time
                terminal_positive = terminal_positive_bool.float()
                # At the terminal anchor and every later target, frame 15 is
                # the sole temporal owner. Clear the local forward owner from
                # frame 12 before enabling terminal Gaussians.
                terminal_target_active = terminal_positive_bool[:, :1, :]
                final_mask[:, :terminal_start, :] = (
                    final_mask[:, :terminal_start, :]
                    & ~terminal_target_active
                )
                forward_ms3_flat = rearrange(gs_params["forward_ms3"][..., :self.ms3_deg * 3], "b t v h w c -> b (t v h w) c")
                forward_ms3_terminal = repeat(forward_ms3_flat[:, terminal_start:terminal_end, :], "b ... -> (b t) ...", t=tgt_t)
                terminal_v0 = forward_ms3_terminal[:, :, 0:3]
                terminal_a0 = forward_ms3_terminal[:, :, 3:6] if self.ms3_deg >= 2 else torch.zeros_like(terminal_v0)
                terminal_j0 = forward_ms3_terminal[:, :, 6:9] if self.ms3_deg >= 3 else torch.zeros_like(terminal_v0)
                terminal_dynamic = classify_terminal_dynamic(
                    terminal_v0, self.terminal_dynamic_velocity_threshold
                ).unsqueeze(-1).float()
                terminal_displacement = ms3_displacement(
                    terminal_v0, terminal_a0, terminal_j0, terminal_tdiff
                ) * terminal_dynamic * terminal_positive
                final_mask[:, terminal_start:terminal_end, :] = final_mask[:, terminal_start:terminal_end, :] | terminal_positive_bool
            ...
            if not static_render:
                ...
                if terminal_displacement is not None:
                    means_batched[:, terminal_start:terminal_end, :] = means_batched[:, terminal_start:terminal_end, :] + terminal_displacement
        ...
        if task_semantic_batched is not None:
            with torch.autocast("cuda", enabled=False):
                rendered_task_semantic_color, _, _ = self.rasterization_func(
                    ...
                    scales=scales_batched.float(),
                    opacities=opacities_batched.float(),
                    colors=task_semantic_batched.float(),
                    viewmats=viewmats_batched,
                    Ks=Ks_batched,
                    width=tgt_w,
                    height=tgt_h,
                ...
                task_sem_logits, _ = rendered_task_semantic_color.split([4, 1], dim=-1)
            output_dict["rendered_task_semantic_logits"] = task_sem_logits.view(b, tgt_t, tgt_v, tgt_h, tgt_w, 4)
            output_dict["rendered_task_semantic"] = task_sem_logits.view(b, tgt_t, tgt_v, tgt_h, tgt_w, 4).argmax(dim=-1)

        if self.use_ms3_motion and self.terminal_context_extrapolation:
            with torch.autocast("cuda", enabled=False):
                rendered_ms3_color, _, _ = self.rasterization_func(
                    means=means_batched.float(),
                    quats=quats_batched.float(),
                    scales=scales_batched.float(),
                    opacities=opacities_batched.float(),
                    colors=target_ms3_batched.float(),
                    viewmats=viewmats_batched,
                    Ks=Ks_batched,
                    width=tgt_w,
                    height=tgt_h,
                ...
                rendered_ms3, _ = rendered_ms3_color.split([9, 1], dim=-1)
            output_dict["rendered_target_ms3"] = rendered_ms3.view(b, tgt_t, tgt_v, tgt_h, tgt_w, 9)
    ...
    def forward(self,
        ...
        if aggregator_kv_cache_list is not None and t != 1:
            pass
        if self.emit_terminal_perception_tokens:
            if v != self.num_cams or v != 3:
                pass
            if (h, w) != (320, 240):
                pass
            expected_timesteps = 1 if aggregator_kv_cache_list is not None else 6
        ...
        with torch.cuda.amp.autocast(enabled=False):
            ...
            if self.use_sky_token:
                ...
                if self.terminal_context_extrapolation:
                    sky_token = sky_token[:, -v:]
            ...
            if self.use_last_token:
                ...
                if self.emit_terminal_perception_tokens:
                    latest_perception_tokens = _select_latest_perception_tokens(
                        aggregated_last_tokens,
                        num_observations=t,
                        num_views=v,
                    )
        ...
        if render_targets:
            output = self.post_processing(data_dict, gs_params, ray_dict=ray_dict,
                                          pred_feat=pred_feat,
                                          pred_task_semantic=pred_task_semantic,
                                          sky_token=sky_token,
            ...
            # A streaming partial observation has no meaningful target render.
            # Return only raw, context-indexed predictions; StreamSession
            # accumulates them and calls post_processing once at the terminal
            # observation.
            output = {
                "ray_dict": ray_dict,
                "gs_params": gs_params,
            ...
            if self.enable_task_semantic_head:
                output["pred_task_semantic"] = pred_task_semantic
            if self.camera_head is not None:
        ...
        if self.emit_terminal_perception_tokens:
            output["latest_perception_tokens"] = latest_perception_tokens
    ...
    def post_processing(self, data_dict, gs_params, time_step=5, ray_dict=None,
                        pred_feat=None, pred_task_semantic=None, sky_token=None, affine_tokens=None,
        ...
        step = self.render_target_chunk_size

        if data_dict["target_camtoworlds"].shape[1] <= step:
            # Rendering the results of context frame aggregation.
            render_results = self.forward_renderer(gs_params, data_dict, feats=pred_feat, task_semantic=pred_task_semantic, time_step=time_step, static_render=static_render)
            ...
            for chunk_start in range(0, data_dict["target_camtoworlds"].shape[1], step):
                ...
                chunk_render_results = self.forward_renderer(gs_params, chunk_data_dict, feats=pred_feat, task_semantic=pred_task_semantic, time_step=time_step, static_render=static_render)
                if chunk_start == 0:
        ...
        if self.enable_task_semantic_head:
            output["pred_task_semantic"] = pred_task_semantic
            output["rendered_task_semantic_logits"] = render_results.get("rendered_task_semantic_logits")
            output["rendered_task_semantic"] = render_results.get("rendered_task_semantic")
        if self.terminal_context_extrapolation and self.use_ms3_motion:
            output["rendered_target_ms3"] = render_results.get("rendered_target_ms3")
        # context
        if self.render_context_view:
    ...
    def save_gs_params_to_ply(self, data_dict, render_results, target_sky, affine, \
        ...
        for t_idx in range(tgt_t):
            ...
            if self.with_feat:
                ...
                raise NotImplementedError("LSeg feat visualization removed in woLSeg variant")
