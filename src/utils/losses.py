...
def compute_loss(output_dict, input_dict, target_dict, args=None, lpips_loss=None, data_iter_step=None):
    if getattr(args, "stream25_reconstruction_loss", False):
        ...
        class_weights = getattr(args, "stream25_semantic_class_weights", None)
        if class_weights is not None:
            class_weights = torch.as_tensor(class_weights, dtype=torch.float32)

        return compute_stream25_loss(
            output_dict,
            target_dict,
            ...
            ms3_scales=stream25_ms3_scales_from_args(args),
            ms3_tail_fraction=getattr(
                args, "stream25_ms3_tail_fraction", 0.10
            ),
            ms3_tail_weight=getattr(args, "stream25_ms3_tail_weight", 1.0),
            ball_depth_tail_fraction=getattr(
                args, "stream25_ball_depth_tail_fraction", 0.10
            ),
