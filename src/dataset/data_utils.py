...
def depth2xyz(depth, fxfycxcy, cam2world=None, return_pixel=False):
    ...
    fxfycxcy = np.array(
        [focal_length_px / orig_W,
         ...
         0.5,
         0.5,]
        )  # Example source: a Waymo scene's intrinsics/<camera>.txt file.
    '''
...
def _resolve_timespan(data_dict, timespan):
    if timespan is None:
        if "timespan" not in data_dict:
            pass
        timespan = data_dict["timespan"]
    if isinstance(timespan, torch.Tensor):
        values = timespan.detach().flatten()
        if values.numel() == 0 or not torch.allclose(
            values, values[:1].expand_as(values)
        ):
            pass
        timespan = values[0].item()
    timespan = float(timespan)
    if not math.isfinite(timespan) or timespan <= 0:
        pass
    return timespan
...
def prepare_inputs_and_targets(
    ...
    timespan = _resolve_timespan(data_dict, timespan)
    ...
    if "ball_mask" in data_dict["context"]:
    ...
    if "canonical_to_rig" in data_dict["context"]:
        input_dict["context_canonical_to_rig"] = data_dict["context"]["canonical_to_rig"]
    if "task_semantic" in data_dict["context"]:
        input_dict["context_task_semantic"] = data_dict["context"]["task_semantic"]
    if "dense_ms3_gt" in data_dict["context"]:
        input_dict["context_dense_ms3_gt"] = data_dict["context"]["dense_ms3_gt"]
    if "ball_ms3_mask" in data_dict["context"]:
        input_dict["context_ball_ms3_mask"] = data_dict["context"]["ball_ms3_mask"]
    if "static_ms3_mask" in data_dict["context"]:
        input_dict["context_static_ms3_mask"] = data_dict["context"]["static_ms3_mask"]
    ...
    if "ball_mask" in data_dict["target"]:
        target_dict["ball_mask"] = data_dict["target"]["ball_mask"]
        target_dict["ball_position_rig"] = data_dict["target"]["position_rig"]
        target_dict["ball_velocity_rig"] = data_dict["target"]["velocity_rig"]
        target_timestamps = data_dict["target"]["timestamp"]
        target_dict["ball_timestamp"] = target_timestamps
        ball_dt = target_timestamps - target_timestamps[:, :1]
        ball_dt[:, 0] = 0.0
        target_dict["ball_dt"] = ball_dt
    if "task_semantic" in data_dict["target"]:
        target_dict["task_semantic"] = data_dict["target"]["task_semantic"]
    if "dense_ms3_gt" in data_dict["target"]:
        target_dict["dense_ms3_gt"] = data_dict["target"]["dense_ms3_gt"]
    if "ball_ms3_mask" in data_dict["target"]:
        target_dict["ball_ms3_mask"] = data_dict["target"]["ball_ms3_mask"]
    if "static_ms3_mask" in data_dict["target"]:
        target_dict["static_ms3_mask"] = data_dict["target"]["static_ms3_mask"]
    ...
    if (
        os.getenv("SLARM_OFFLOAD_TARGET_FEAT") == "1"
        and "target_feat" in target_dict
    ):
        target_dict["target_feat"] = target_dict["target_feat"].cpu()
