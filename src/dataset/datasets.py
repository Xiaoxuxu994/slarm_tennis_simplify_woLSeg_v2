...
class PerceptualModelDataset(Dataset):
    def __init__(
        ...
        num_max_cams: Literal[1, 2, 3, 5, 6, 7] = 3,
        ...
        if not math.isfinite(self.timespan) or self.timespan <= 0:
            pass
    ...
    def get_frame(
        ...
        for camera in camera_list:
            ...
            if self.online_feat:
                if dataset_name == "driving_sim" or dataset_name == "b2d" or dataset_name.startswith("ball_catch"):
                    # driving_sim/b2d/ball_catch* has no downsampled directory, use original RGB path directly for online feature extraction
            ...
            if dataset_name in ["waymo", "nuscenes", "argoverse2", "driving_sim"]:
                ...
                sky_masks.append(sky)
            elif dataset_name.startswith("ball_catch"):
                sky = torch.zeros(self.target_size[0], self.target_size[1]).float()
                sky_masks.append(sky)
            ...
            if self.load_semantic_label:
                ...
                if dataset_name == "waymo":
                    ...
                    if raw_ball_semantic is None:
                        semantic_path = img_path.replace("vis/color", "vis/semantic").replace(".jpg", ".png")
                        semantic = cv2.imread(semantic_path, cv2.IMREAD_UNCHANGED)
                        semantic = cv2.resize(semantic, self.target_size[::-1], interpolation=cv2.INTER_NEAREST)
                    else:
                        semantic = raw_ball_semantic
                    semantic = to_tensor(semantic.astype(np.float32)).float()
                    has_semantic = torch.tensor(True)
...
class Stream25Dataset(PerceptualModelDataset):
    """Stream25 streaming-reconstruction dataset (spec 4, 5.1, 6.1).

    Context is always ``[0, 3, 6, 9, 12, 15]`` for every named native view; training returns the
    seven scheduler targets, validation/test return all 25 targets. The
    four-class task semantic is loaded into ``task_semantic`` separately from the
    ...
    def __init__(self, *args, training: bool = True, **kwargs):
        ...
        if self.num_target_timesteps != 7:
            pass
        self._ball_visibility_by_scene: list[torch.Tensor] = []
        for scene_json in self.annotations:
            dataset_name = scene_json["dataset"]
            camera_contract = DATASET_DICT[dataset_name]["camera_list"].get(
                self.num_max_cams
            )
            if camera_contract is None:
                pass
            scene_camera_list = scene_json.get("camera_list")
            if (
                not isinstance(scene_camera_list, list)
                or scene_camera_list[: len(camera_contract)] != camera_contract
            ):
                pass
            visible_mask_by_camera = scene_json.get(
                "ball_visible_mask_by_camera"
            )
            if visible_mask_by_camera is not None:
                visible_mask_by_camera = {
                    camera: visible_mask_by_camera[camera]
                    for camera in camera_contract
                }
            visible_frames_by_camera = scene_json.get(
                "ball_visible_frames_by_camera"
            )
            if visible_frames_by_camera is not None:
                visible_frames_by_camera = {
            ...
            if (
                ...
                or len(timestamps) <= terminal_frame
            ):
                pass
            recorded_timespan = float(timestamps[terminal_frame]) - float(
                timestamps[STREAM25_ALL_TARGET_FRAMES[0]]
            )
            if (
                not math.isfinite(recorded_timespan)
                or recorded_timespan <= 0
                or not math.isclose(
                    float(self.timespan),
                    recorded_timespan,
                    rel_tol=0.0,
    ...
    def _preflight_stream25_semantic_visibility(
        ...
        declared_visibility: torch.Tensor,
    ) -> None:
        """Verify every declared frame-eye against its semantic artifact."""
        semantic_paths = scene_json.get("task_semantic_path")
        if not isinstance(semantic_paths, dict) or any(
            camera not in semantic_paths for camera in camera_list
        ):
            pass
        num_frames = scene_json["num_timesteps"]
        for camera_index, camera in enumerate(camera_list):
            paths = semantic_paths.get(camera)
            if not isinstance(paths, list) or len(paths) != num_frames:
                pass
            for frame_idx, relative_path in enumerate(paths):
                ...
                if semantic is None:
                    pass
                actual_visible = bool(np.any(semantic == 1))
                if actual_visible != bool(
                    declared_visibility[frame_idx, camera_index].item()
                ):
                    pass
    ...
    def __getitem__(self, sample_key, context_frame_idx: int = -1, return_all: bool = False) -> Dict[str, Any]:
        if isinstance(sample_key, tuple):
            scene_index, optimizer_step = sample_key
        else:
        ...
        if return_all or not self.training:
            ...
            target_frames = self._scheduler.targets(optimizer_step, scene_index)

        context_list = [
            self._get_stream25_frame(
                scene_json, f, source_frame_idx, time_in_seconds, scene_index, optimizer_step
            )
        ...
        sample = {
            "context": self._collate_stream25_frames(context_list),
            "target": self._collate_stream25_frames(target_list),
            "dataset_name": scene_json["dataset"],
            "scene_id": scene_json["scene_id"],
            "scene_name": scene_json["scene_name"],
            "width": self.target_size[1],
        ...
        # Restore integer / boolean dtypes that to_float_tensor flattened.
        sample["context"]["ball_mask"] = sample["context"]["ball_mask"].bool()
        sample["target"]["ball_mask"] = sample["target"]["ball_mask"].bool()
        sample["context"]["ball_visible"] = sample["context"]["ball_visible"].bool()
        sample["target"]["ball_visible"] = sample["target"]["ball_visible"].bool()
        sample["context"]["ball_region_eligible"] = sample["context"][
            "ball_region_eligible"
        ].bool()
        sample["target"]["ball_region_eligible"] = sample["target"][
            "ball_region_eligible"
        ].bool()
        sample["context"]["task_semantic"] = sample["context"]["task_semantic"].long()
        sample["target"]["task_semantic"] = sample["target"]["task_semantic"].long()
        sample["context"]["ball_ms3_mask"] = sample["context"]["ball_ms3_mask"].bool()
        sample["target"]["ball_ms3_mask"] = sample["target"]["ball_ms3_mask"].bool()
        sample["context"]["static_ms3_mask"] = sample["context"]["static_ms3_mask"].bool()
        sample["target"]["static_ms3_mask"] = sample["target"]["static_ms3_mask"].bool()
        return sample

    def _get_stream25_frame(
        self, scene_json, frame_idx, source_frame_idx, time_in_seconds, scene_index, optimizer_step
    ):
        frame = self.get_frame(scene_json, frame_idx, source_frame_idx)
        dataset_name = scene_json["dataset"]
        camera_list = DATASET_DICT[dataset_name]["camera_list"][self.num_max_cams]
        ...
        for camera in camera_list:
            ...
            sem_tensor = torch.from_numpy(raw_semantic.astype(np.int64))
            task_semantics.append(sem_tensor)
            ball_masks.append(sem_tensor == 1)
        frame["task_semantic"] = torch.stack(task_semantics)  # (num_cams, H, W)
        frame["ball_mask"] = torch.stack(ball_masks)  # (num_cams, H, W) bool
        actual_visibility = frame["ball_mask"].flatten(1).any(dim=1)
        declared_visibility = self._ball_visibility_by_scene[scene_index][frame_idx]
        if not torch.equal(actual_visibility, declared_visibility):
            mismatched_cameras = [
                camera
                for camera_index, camera in enumerate(camera_list)
                if actual_visibility[camera_index]
                != declared_visibility[camera_index]
            ]
            pass
        ...
        frame["velocity_rig"] = torch.tensor(
            traj_frame["velocity_rig"], dtype=torch.float32
        )  # (3,)

        # Gaussian means use the reference camera's canonical-FLU frame while
        ...
        canonical_to_rig = (
            np.linalg.inv(rig_to_world)
            @ ref_camera_to_world
            @ np.linalg.inv(canonical_to_flu)
        )
        frame["canonical_to_rig"] = torch.from_numpy(canonical_to_rig).float()

        velocity_canonical, gravity_canonical = rig_ms3_to_canonical(
            traj_frame["velocity_rig"],
            MS3_GRAVITY_RIG,
            canonical_to_rig,
        )
        ms3 = build_dense_ms3_gt(
            frame["task_semantic"],
            frame["depth"],
            velocity_canonical,
            gravity_rig=gravity_canonical,
        )
        frame["dense_ms3_gt"] = ms3["dense_ms3"]  # (num_cams, H, W, 9)
        frame["ball_ms3_mask"] = ms3["ball_mask"]  # (num_cams, H, W) bool
        ball_counts = ms3["ball_mask"].flatten(1).sum(dim=1)
        offscreen_static_cap = max(1, int(ball_counts.max().item()))
        static_sample_caps = tuple(
            int(count.item()) if count.item() > 0 else offscreen_static_cap
            for count in ball_counts
        )
        step_for_seed = optimizer_step if optimizer_step is not None else 0
        frame["static_ms3_mask"] = select_static_samples(
            ms3["static_valid_mask"],
            ms3["ball_mask"],
            seed_key=(step_for_seed, scene_index, frame_idx),
            sample_caps=static_sample_caps,
        )  # (num_cams, H, W) bool

        dt = time_in_seconds[frame_idx] - time_in_seconds[source_frame_idx]
        frame["time"] = torch.tensor(
            [dt] * self.num_max_cams, dtype=torch.float32
        )  # (num_cams,)
        frame["timestamp"] = torch.tensor(
            time_in_seconds[frame_idx], dtype=torch.float32
        )
        return frame

    @staticmethod
    def _collate_stream25_frames(frame_list):
        collated = default_collate(frame_list)
        # Per-frame / per-camera tensors keep their time axis; flattened tv
        # tensors (image, camtoworld, intrinsics, depth, sky_masks, time,
        # timestamp, frame_idx) are concatenated along dim 0 so
        # prepare_inputs_and_targets can reshape (b, tv, ...) -> (b, t, v, ...).
        preserve_time_axes = {
            "ball_mask",
            "ball_visible",
            "ball_region_eligible",
        ...
        for key, value in collated.items():
            if (
                key not in preserve_time_axes
                and isinstance(value, torch.Tensor)
                and len(value.shape) >= 2
            ):
                collated[key] = torch.cat([v for v in value], dim=0)
        return collated


class Stream25EvalDataset(Stream25Dataset):
    """Evaluation view of the Stream25 dataset: returns all 25 targets."""
